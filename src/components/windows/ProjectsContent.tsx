...
import { Folder, FolderOpen, ExternalLink, FileText, Code2, ChevronRight, ChevronDown } from "lucide-react";
import projectsData from "@/data/projects.json";

// TODO: make a mock source folder in the primary languages of these projects
// ie. have a cargo crate for bagel
export default function ProjectsContent() {
  const [selectedProject, setSelectedProject] = useState<any>(projectsData[0]);
  const [openFolders, setOpenFolders] = useState<Record<string, boolean>>({
    [projectsData[0].name]: true
  });

  const toggleFolder = (name: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setOpenFolders(prev => ({ ...prev, [name]: !prev[name] }));
    const project = projectsData.find((p: any) => p.name === name);
                ...
                <div key={project.name} className="mt-0.5">
                  <div 
                    className={`flex items-center gap-1.5 py-1 px-1 cursor-pointer transition-colors ${
                      isSelected && !isOpen ? "bg-navy text-canvas font-bold" : "hover:bg-navy/10 text-navy"
                    }`}
                  ...
                  {isOpen && (
                    <div className="pl-5 space-y-0.5 py-0.5">
                      <div 
                        className={`flex items-center gap-1.5 py-1 px-1 cursor-pointer transition-colors ${
                          isSelected ? "bg-navy text-canvas font-bold" : "hover:bg-navy/10 text-navy"
                        }`}
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-canvas">
        <div className="flex bg-canvas border-b border-navy overflow-x-auto scrollbar-none">
          <div className="flex items-center gap-2 px-3 py-2 bg-navy text-canvas font-bold min-w-fit cursor-default">
            <FileText size={14} />
            <span className="truncate">README.md</span>
            <span className="ml-2 text-[10px] opacity-70 shrink-0">~/{selectedProject?.name}</span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 scrollbar-thin scrollbar-thumb-navy/20">
          <div className="max-w-2xl">
            <div className="mb-6 border-y border-dashed border-navy/30 py-3 text-navy/70">
              <div><span className="font-bold text-navy">title:</span> "{selectedProject?.name}"</div>
              <div><span className="font-bold text-navy">year:</span> {selectedProject?.year}</div>
            </div>
            ...
            {selectedProject?.tags && selectedProject.tags.length > 0 && (
              <div className="mb-6 mt-10">
                <div className="font-bold mb-2 uppercase text-[10px] tracking-widest text-navy/60">## Tech Stack</div>
                <div className="flex flex-wrap gap-2">
                  {selectedProject.tags.map((tag: string) => (
                    <span key={tag} className="px-2 py-1 bg-transparent border border-navy text-navy rounded-none text-[11px]">
                      {tag}
                    </span>
                  ))}
                </div>
